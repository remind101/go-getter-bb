...
This is a fork purely for educational/learning. 

The main branch contains the the following bug:

The file ordering tar file bug is an issue where go-getter won't decompress tar files where there are individual
files that occur before directories (the order in which the files occur is just based on their naming). The
specification for tar files doesn't list any sort of file ordering requirement, so go-getter has an unnecessary
ordering constraint that the candidate needs to be removed to fix the issue.

The following test files have been included to bring the bug forward:

* testdata/decompress-tgz/ordering.tar.gz
* testdata/decompress-tbz2/ordering.tar.bz2
